  ...
  this.on("getChatRagResponse", async (req) => {
    try {
      ...
      if (category === "leave-request-query") {
        ...
        let data = await sf_connection_util.getPeersVacationTimeByUserId(
          userId,
        ...
        data.forEach((item) => {
          ...
          item.vacations.forEach((vacation) => {
            formattedData.push([
              getFormattedDate(vacation.startDate),
              getFormattedDate(vacation.endDate),
            ]);
          });
          if (formattedData.length > 0) {
            teamLeaveDates[item.displayName] = formattedData;
          }
        });
