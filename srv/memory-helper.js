...
async function insertMessage(
  ...
  const updateConversationStatus = await UPDATE(conversationEntity)
    .set`last_update_time = ${messageTime}`.where`cID = ${conversationId}`;
...
async function handleMemoryBeforeRagCall(
  ...
  try {
    ...
    const isConversationPresent = await SELECT.from(Conversation).where({
      cID: conversationId,
    });
    ...
    if (isConversationPresent.length > 0) {
      ...
      const messageSelectStmt = await SELECT.from(Message)
