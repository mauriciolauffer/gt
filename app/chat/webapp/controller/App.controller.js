sap.ui.define([
  ...
  return BaseController.extend("chat.controller.App", {
    onDeleteChat: async function(evt) {
      ...
      objectBinding.setParameter('sessionId', uiModel.getProperty("/sessionId"));
      await objectBinding.execute();
      this.getView().getModel("chat").setProperty("/", []);
      uiModel.setProperty("/sessionId", crypto.randomUUID());
      this.setAppBusy(false);
    },

    onSendMessage: async function (evt) {
      this.setAppBusy(true);
      const userMessage = this.addUserMessageToChat(evt.getParameter("value"));
      const payload = {
      ...
      try {
        ...
        this.addSystemMessageToChat({content: "Error connecting to AI...", messageTime: new Date().toJSON()})
        logger.error(err);
      }
      this.setAppBusy(false);
    },
    ...
    askAiAssistent: async function (payload) {
      const url = this.getOwnerComponent().getManifestEntry("sap.app").dataSources.mainService.uri + "getAiResponse";
