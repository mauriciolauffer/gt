sap.ui.define(
  ...
  function (
    ...
    return Controller.extend("chatting.controller.LeftScreen", {
      onInit: function () {
        // Set User Info Model
        // this.getUserInfo();
      ...
      onHandleConversationDelete: function (oEvent) {
        ...
        MessageBox.warning(`This will delete ${conversationTitle}`, {
      ...
      requestConversationDelete: function (conversationID) {
        const settings = {
          url:
            this.getBaseURL() +
            `/odata/v4/chat/Conversation(${conversationID})`,
          method: "DELETE",
          headers: {
      ...
      onUploadFileBtnSelect: function (oEvent) {
        this.fileUploadFragment ??= this.loadFragment({
          name: "chatting.view.FileUploading",
        });
      ...
      onManageFileBtnSelect: function () {
        this.fileManagementFragment ??= this.loadFragment({
          name: "chatting.view.FileManagement",
        });
      ...
      uploadContent: function (item, id) {
        var url =
          this.getBaseURL() +
          `/odata/v4/embedding-storage/Files(${id})/content`;
      ...
      requestFileDownload: function (fileID) {
        const settings = {
          url:
            this.getBaseURL() +
            `/odata/v4/embedding-storage/Files(${fileID})/content`,
          method: "GET",
          xhrFields: {
            responseType: "blob",
          },
      ...
      onGenerateVectorBtnClick: function (oEvent) {
        ...
        while (
          clickedControl &&
          !(clickedControl instanceof UploadSetwithTableItem)
        ) {
        ...
        this.requestEmbeddingGeneration(pdfFileID)
          ...
          .catch((error) => {
            this.byId("fileManagementFragment").setBusy(false);
            MessageToast.show(
              "Embeddings generation failed, please try again.",
            );
          });
      },
      ...
      requestEmbeddingGeneration: function (pdfFileID) {
        ...
        return new Promise((resolve, reject) => {
          $.ajax({
            url:
              this.getBaseURL() + "/odata/v4/embedding-storage/storeEmbeddings",
            type: "POST",
      ...
      onDeleteFiles: function (oEvent) {
        ...
        this.requestFileDelete(fileID)
          .then((result) => {
            ...
            MessageToast.show(
              `File ${fileName} with ID ${fileID} successfully deleted`,
            );
          })
          .catch((error) => {
            ...
            this.byId("fileManagementFragment").setBusy(false);
            MessageToast.show(
              `File ${fileName} with ID ${fileID} deletion failed`,
            );
          });
      },
      ...
      requestFileDelete: function (fileID) {
        const settings = {
          url:
            this.getBaseURL() + `/odata/v4/embedding-storage/Files(${fileID})`,
          method: "DELETE",
        };
      ...
      requestEmbeddingDelete: function () {
        const settings = {
          url:
            this.getBaseURL() +
            "/odata/v4/embedding-storage/deleteEmbeddings()",
          method: "GET",
        };
